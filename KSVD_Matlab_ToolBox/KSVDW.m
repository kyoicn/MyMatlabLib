function [Dictionary,output] = KSVDW(...
    Data,... % an nXN matrix that contins N signals (Y), each of dimension n.
    param,...
    Weights) % [=ones(n, 1)] an n-by-1 binary vector 
             % of weights assigned to each elements in signals.
% =========================================================================
%                      K-SVD algorithm with Weights
% =========================================================================
...
% 
% Adding weights while sparse coding stage.
% Modifier: Liwen XU [kyoi.cn@gmail.com]
% =========================================================================
% INPUT ARGUMENTS:
% Data                         an nXN matrix that contins N signals (Y), each of dimension n. 
...
if (nargin < 3) Weights = ones(n, 1); end
% -------------------------
...
% prepare weighted dictionary and data for sparse coding
idx = find(Weights == 1);
wData = Data(idx, :);
wDictionary = Dictionary(idx, :);
wDictionary = wDictionary*diag(1./sqrt(sum(wDictionary.*wDictionary)));
wDictionary = wDictionary.*repmat(sign(wDictionary(1,:)),size(wDictionary,1),1);
if (param.preserveDCAtom>0)
    wFixedDictionaryElement = FixedDictionaryElement(idx);
else
    wFixedDictionaryElement = [];
end
...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        CoefMatrix = OMP([wFixedDictionaryElement,wDictionary],wData, param.L);
    else 
        ...
        CoefMatrix = OMPerr([wFixedDictionaryElement,wDictionary],wData, param.errorGoal);
        param.L = 1;
    end
...
%     fprintf('%d-%d, %d-%d, %d-%d\n', size(Data, 1), size(Data, 2), size(Dictionary, 1), size(Dictionary, 2), size(CoefMatrix, 1), size(CoefMatrix, 2));
