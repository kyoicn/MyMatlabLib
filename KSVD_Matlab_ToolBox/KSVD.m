...
bestError = inf;
bestDictionary = zeros(size([FixedDictionaryElement,Dictionary]));
for iterNum = 1:param.numIteration
    ...
    if (iterNum>1)
        ...
        if param.displayProgress
            if (param.errorFlag==0)
    %             output.totalerr(iterNum-1) = sqrt(sum(sum((Data-[FixedDictionaryElement,Dictionary]*CoefMatrix).^2))/prod(size(Data)));
                disp(['Iteration   ',num2str(iterNum),'   Total error is: ',num2str(output.totalerr(iterNum-1)), ' BestErr: ', num2str(bestError)]);
            else
    ...
    if (iterNum > 1 && bestError > output.totalerr(iterNum - 1))
        bestError = output.totalerr(iterNum - 1);
        bestDictionary = [FixedDictionaryElement,Dictionary];
    end
