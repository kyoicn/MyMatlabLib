...
function [samples samplex] = lcaf_sample(m, cost, r)
    dim = size(cost);
%     ndim = length(dim);
    n = prod(dim);
    if (n == 0) samples = []; samplex = samples; return;
    elseif (n <= m) samples = ones(dim); samplex = samples; return;
    end
    ...
    for seedi = 1 : m
        ...
        idx = find(dis(:) <= r^2);
        alp = 1;
        offset = randsamplewtr(length(idx), 1, pcost(idx).^(-alp)/sum(pcost(idx).^(-alp)));
